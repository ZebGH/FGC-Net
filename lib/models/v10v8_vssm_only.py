...
# 💡 [新增] 检测分支的深度局部特征提取 (克制使用 DCN)
    ...
    [1, Conv, [96, 64, 3, 1]],  # 8
    [-1, C3, [64, 64]],  # 9  (DA S8)
    [2, Conv, [192, 128, 3, 1]],  # 10
    [-1, C2f, [128, 128]],  # 11 (DA S16)
    [3, Conv, [384, 256, 3, 1]],  # 12
    [-1, ASPP, [256, 256]],  # 13 (DA S32)

    # LL Encoder
    [1, Conv, [96, 64, 3, 1]],  # 14
    [-1, SC, [64, 64]],  # 15 (LL S8)
    [2, Conv, [192, 128, 3, 1]],  # 16
    [-1, SC, [128, 128]],  # 17 (LL S16)
    [3, Conv, [384, 256, 3, 1]],  # 18
    [-1, SC, [256, 256]],  # 19 (LL S32)
    ...
    # [[9,14,20], HA_FIM_s, [256, 128, 128, 128, 128, 128, 256, 128, 128]],  # 24

    # FIM @ Stride 32 (轻量级全局反哺)
    # [[7, 13, 19], Concat, [1]],  # 28
    # [-1, Conv, [768, 384, 1, 1]],  # 29
    # [-1, Split, [128, 128, 128]],  # 30
    # [[10,16,22], HA_FIM_s, [256, 256, 256, 128, 128, 128, 512, 256, 256]],  # 25
    ...
    [[21, 23, 25], Detect,
     [1, [[3, 9, 5, 11, 4, 20], [7, 18, 6, 39, 12, 31], [19, 50, 38, 81, 68, 157]], [128, 256, 512]]],  # 26

    # === 分割多尺度金字塔融合 (Top-Down) ===
    # 1. 行驶区域 (DA) FPN
    # [25, SelectDA, []],  # 36 (DA32) 256
    [13, Upsample, [None, 2, 'nearest']],  # 27 (DA32 -> 16)
    # [24, SelectDA, []],  # 38 (DA16) 128
    [[11, 27], Concat, [1]],  # 28 融合
    [-1, Conv, [384, 128, 3, 1]],  # 29 (Fused_DA16)
    [-1, Upsample, [None, 2, 'nearest']],  # 30 (Fused_DA16 -> 8)
    # [23, SelectDA, []],  # 42 (DA8)
    [[9, 30], Concat, [1]],  # 31 终极融合
    [-1, Conv, [192, 128, 3, 1]],  # 32 (Fused_DA8)

    # 2. 车道线 (LL) FPN
    # [25, SelectLL, []],  # 45 (LL32)
    [19, Upsample, [None, 2, 'nearest']],  # 33 (LL32 -> 16)
    # [24, SelectLL, []],  # 47 (LL16)
    [[17, 33], Concat, [1]],  # 34 融合
    [-1, Conv, [384, 128, 3, 1]],  # 35 (Fused_LL16)
    [-1, Upsample, [None, 2, 'nearest']],  # 36 (Fused_LL16 -> 8)
    # [23, SelectLL, []],  # 51 (LL8)
    [[15, 36], Concat, [1]],  # 37 终极融合
    [-1, Conv, [192, 128, 3, 1]],  # 38 (Fused_LL8)

    # === 空间一致性约束 (DSSCM) ===
    # 利用最高分辨率的 Det8 掩膜去修剪已经完全融合好的 Fused_DA8 和 Fused_LL8
    # [[24, 35, 41], DSSCM, [128, 128, 128]],  # 42 -> 返回 [DA_mod, LL_mod]
    # [-1, Select0, []],  # 43 (DA_mod8)
    # [42, Select1, []],  # 44 (LL_mod8)
    ...
    # === DA 最终解码输出 ===
    [32, Upsample_PixelShuffle, [128, 128]],  # 39 (->160x160)
    [-1, Conv, [128, 64, 3, 1]],  # 40
    [0, Select0, []],  # 41 (获取 Backbone C1: 160x160)
    [[40, 41], Concat, [1]],  # 42
    [-1, Conv, [112, 64, 3, 1]], #43
    [-1, SEAttention, [64, 16]],  # 44

    [-1, Upsample_PixelShuffle, [64, 64]],  # 45 (->320x320)
    [-1, Conv, [64, 32, 3, 1]],  # 46
    [-1, Upsample_PixelShuffle, [32, 32]],  # 47 (->640x640)
    [-1, Conv, [32, 16, 3, 1]],  # 48
    [-1, Conv, [16, 3, 3, 1]],  # 49 🎯 (DA 最终多分类输出)

    # === LL 最终解码输出 ===
    [38, Upsample_PixelShuffle, [128, 128]],  # 50 (->160x160)
    [-1, Conv, [128, 64, 3, 1]],  # 51
    [[51, 41], Concat, [1]],  # 52 (复用 C1)
    [-1, Conv, [112, 64, 3, 1]], # 53
    [-1, SEAttention, [64, 16]],  # 54

    [-1, Upsample_PixelShuffle, [64, 64]],  # 55 (->320x320)
    [-1, Conv, [64, 32, 3, 1]],  # 56
    [-1, Upsample_PixelShuffle, [32, 32]],  # 57 (->640x640)
    [-1, Conv, [32, 16, 3, 1]],  # 58
    [-1, Conv, [16, 3, 3, 1]]  # 59 🎯 (LL 最终多分类输出)
]
