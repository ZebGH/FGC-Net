...
Net = [
    [32, 64, 74],  # output index[det, da, ll]
    [-1, VSSMBackbone, []],
...
# 💡 [新增] 检测分支的深度局部特征提取 (克制使用 DCN)
    ...
    [[5,9,15], HA_FIM, [256, 64, 64, 128, 128, 128, 128, 64, 64]],  # 23

    # FIM @ Stride 16 (轻量级全局反哺)
    # [[6, 11, 17], Concat, [1]],  # 24
    # [-1, Conv, [512, 384, 1, 1]],  # 25
    # [-1, Split, [128, 128, 128]],  # 26
    [[6,11,17], HA_FIM, [256, 128, 128, 128, 128, 128, 256, 128, 128]],  # 24

    # FIM @ Stride 32 (轻量级全局反哺)
    # [[7, 13, 19], Concat, [1]],  # 28
    # [-1, Conv, [768, 384, 1, 1]],  # 29
    # [-1, Split, [128, 128, 128]],  # 30
    [[7,13,19], HA_FIM, [256, 256, 256, 128, 128, 128, 512, 256, 256]],  # 25

    # === 检测头 (Detection Head) ===
    [20, SelectDet, []],  # 26 (Det8) 128
    [21, SelectDet, []],  # 27 (Det16)
    [22, SelectDet, []],  # 28 (Det32)

    [[5, 23], Concat, [1]], # 29
    [-1, Conv, [384, 128, 1,1]], #30
    [[6, 24], Concat, [1]], #31
    [-1, Conv, [512, 256, 1,1]], #32
    [[7, 25], Concat, [1]], #33
    [-1, Conv, [768, 512, 1,1]], #34
    ...
    # === DA 最终解码输出 ===
    [52, Upsample_PixelShuffle, [128, 128]],  # 57 (->160x160)
    [-1, Conv, [128, 64, 3, 1]],  # 58
    [0, Select0, []],  # 59 (获取 Backbone C1: 160x160)
    [[55, 56], Concat, [1]],  # 60
    [-1, Conv, [112, 64, 3, 1]], #61
    [-1, SEAttention, [64, 16]],  # 62

    [-1, Upsample_PixelShuffle, [64, 64]],  # 63 (->320x320)
    [-1, Conv, [64, 32, 3, 1]],  # 64
    [-1, Upsample_PixelShuffle, [32, 32]],  # 65 (->640x640)
    [-1, Conv, [32, 16, 3, 1]],  # 66
    [-1, Conv, [16, 3, 3, 1]],  # 67 🎯 (DA 最终多分类输出)

    # === LL 最终解码输出 ===
    [53, Upsample_PixelShuffle, [128, 128]],  # 68 (->160x160)
    [-1, Conv, [128, 64, 3, 1]],  # 69
    [[66, 56], Concat, [1]],  # 70 (复用 C1)
    [-1, Conv, [112, 64, 3, 1]], # 71
    [-1, SEAttention, [64, 16]],  # 72

    [-1, Upsample_PixelShuffle, [64, 64]],  # 73 (->320x320)
    [-1, Conv, [64, 32, 3, 1]],  # 74
    [-1, Upsample_PixelShuffle, [32, 32]],  # 75 (->640x640)
    [-1, Conv, [32, 16, 3, 1]],  # 76
    [-1, Conv, [16, 3, 3, 1]]  # 77 🎯 (LL 最终多分类输出)
]
