...
class Split(nn.Module):
    ...
    def forward(self, x):
        assert self.c1 + self.c2 + self.c3 == x.shape[1]
        x_det = x[:, :self.c1]
        x_da = x[:, self.c1:self.c1 + self.c2]
        x_ll = x[:, self.c1 + self.c2:]
        return x_det, x_da, x_ll
