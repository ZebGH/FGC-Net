...
# 💡 [新增] 检测分支的深度局部特征提取 (克制使用 DCN)
    ...
    # === DA 最终解码输出 ===
    [35, Upsample_PixelShuffle, [128, 128]],  # 42 (->160x160)
    [-1, Conv, [128, 64, 3, 1]],  # 43
    [0, Select0, []],  # 44 (获取 Backbone C1: 160x160)
    [[43, 44], Concat, [1]],  # 45
    [-1, Conv, [112, 64, 3, 1]], #46
    [-1, SEAttention, [64, 16]],  # 47

    [-1, Upsample_PixelShuffle, [64, 64]],  # 48 (->320x320)
    [-1, Conv, [64, 32, 3, 1]],  # 49
    [-1, Upsample_PixelShuffle, [32, 32]],  # 50 (->640x640)
    [-1, Conv, [32, 16, 3, 1]],  # 51
    [-1, Conv, [16, 3, 3, 1]],  # 52 🎯 (DA 最终多分类输出)

    # === LL 最终解码输出 ===
    [41, Upsample_PixelShuffle, [128, 128]],  # 53 (->160x160)
    [-1, Conv, [128, 64, 3, 1]],  # 54
    [[54, 44], Concat, [1]],  # 55 (复用 C1)
    [-1, Conv, [112, 64, 3, 1]], # 56
    [-1, SEAttention, [64, 16]],  # 57

    [-1, Upsample_PixelShuffle, [64, 64]],  # 58 (->320x320)
    [-1, Conv, [64, 32, 3, 1]],  # 59
    [-1, Upsample_PixelShuffle, [32, 32]],  # 60 (->640x640)
    [-1, Conv, [32, 16, 3, 1]],  # 61
    [-1, Conv, [16, 3, 3, 1]]  # 62 🎯 (LL 最终多分类输出)
]
