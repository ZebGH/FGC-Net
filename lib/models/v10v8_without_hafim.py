...
# 💡 [新增] 检测分支的深度局部特征提取 (克制使用 DCN)
    ...
    [[5, 8], Concat, [1]], # 23
    [-1, HF_C2f_DCN, [512, 128, 2]], #24
    [[6, 9], Concat, [1]], #25
    [-1, HF_C2f_DCN, [512, 256, 2]], #26
    [[7, 10], Concat, [1]], #27
    [-1, HF_C2f_DCN, [512, 512, 3]], #28

    [[24, 26, 28], Detect,
     [1, [[3, 9, 5, 11, 4, 20], [7, 18, 6, 39, 12, 31], [19, 50, 38, 81, 68, 157]], [128, 256, 512]]],  # 29

    # === 分割多尺度金字塔融合 (Top-Down) ===
    # 1. 行驶区域 (DA) FPN
    # [25, SelectDA, []],  # 36 (DA32) 256
    [16, Upsample, [None, 2, 'nearest']],  # 30 (DA32 -> 16)
    # [24, SelectDA, []],  # 38 (DA16) 128
    [[14, 30], Concat, [1]],  # 31 融合
    [-1, Conv, [384, 128, 3, 1]],  # 32 (Fused_DA16)
    [-1, Upsample, [None, 2, 'nearest']],  # 33 (Fused_DA16 -> 8)
    # [23, SelectDA, []],  # 42 (DA8)
    [[12, 33], Concat, [1]],  # 34 终极融合
    [-1, Conv, [192, 128, 3, 1]],  # 35 (Fused_DA8)

    # 2. 车道线 (LL) FPN
    # [25, SelectLL, []],  # 45 (LL32)
    [22, Upsample, [None, 2, 'nearest']],  # 36 (LL32 -> 16)
    # [24, SelectLL, []],  # 47 (LL16)
    [[20, 36], Concat, [1]],  # 37 融合
    [-1, Conv, [384, 128, 3, 1]],  # 38 (Fused_LL16)
    [-1, Upsample, [None, 2, 'nearest']],  # 39 (Fused_LL16 -> 8)
    # [23, SelectLL, []],  # 51 (LL8)
    [[18, 39], Concat, [1]],  # 40 终极融合
    [-1, Conv, [192, 128, 3, 1]],  # 41 (Fused_LL8)
    ...
    # === DA 最终解码输出 ===
    [43, Upsample_PixelShuffle, [128, 128]],  # 45 (->160x160)
    [-1, Conv, [128, 64, 3, 1]],  # 46
    [0, Select0, []],  # 47 (获取 Backbone C1: 160x160)
    [[46, 47], Concat, [1]],  # 48
    [-1, Conv, [112, 64, 3, 1]], #49
    [-1, SEAttention, [64, 16]],  # 50

    [-1, Upsample_PixelShuffle, [64, 64]],  # 51 (->320x320)
    [-1, Conv, [64, 32, 3, 1]],  # 52
    [-1, Upsample_PixelShuffle, [32, 32]],  # 53 (->640x640)
    [-1, Conv, [32, 16, 3, 1]],  # 54
    [-1, Conv, [16, 3, 3, 1]],  # 55 🎯 (DA 最终多分类输出)

    # === LL 最终解码输出 ===
    [44, Upsample_PixelShuffle, [128, 128]],  # 56 (->160x160)
    [-1, Conv, [128, 64, 3, 1]],  # 57
    [[57, 47], Concat, [1]],  # 58 (复用 C1)
    [-1, Conv, [112, 64, 3, 1]], # 59
    [-1, SEAttention, [64, 16]],  # 60

    [-1, Upsample_PixelShuffle, [64, 64]],  # 61 (->320x320)
    [-1, Conv, [64, 32, 3, 1]],  # 62
    [-1, Upsample_PixelShuffle, [32, 32]],  # 63 (->640x640)
    [-1, Conv, [32, 16, 3, 1]],  # 64
    [-1, Conv, [16, 3, 3, 1]]  # 65 🎯 (LL 最终多分类输出)
]
