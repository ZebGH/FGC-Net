import time
from lib.core.evaluate_bak import ConfusionMatrix,SegmentationMetric
...
def validate(epoch,config, val_loader, val_dataset, model, criterion, output_dir,
    ...
    for batch_i, (img, target, paths, shapes) in tqdm(enumerate(val_loader), total=len(val_loader)):
        ...
        with torch.no_grad():
            ...
            # 1. 从模型输出获取预测类别 (B, C, H, W) -> (B, H, W)
            _, da_predict = torch.max(da_seg_out, 1)
            _, ll_predict = torch.max(ll_seg_out, 1)

            # 2. 直接使用从 dataloader 传来的 ground truth target
            #    它们的形状已经是 [B, H, W]，类型是 long
            da_gt = target[1]
            ll_gt = target[2]

            # 3. 逐个样本处理和评估
            for i in range(nb):  # nb 是 batch size, 在 for 循环开始时已定义
                # 获取当前样本的 padding 信息
                pad_w, pad_h = shapes[i][1][1]
                pad_w = int(pad_w)
                pad_h = int(pad_h)

                # 获取单个样本的预测和标签 (现在是2D张量)
                da_predict_single = da_predict[i]
                da_gt_single = da_gt[i]
                ll_predict_single = ll_predict[i]
                ll_gt_single = ll_gt[i]

                # 获取 letterbox 之后的全尺寸 H, W
            ...
            # 在 batch 循环结束后计算整个 batch 的指标
            da_acc = da_metric.pixelAccuracy()
            da_IoU = da_metric.IntersectionOverUnion()
            da_mIoU = da_metric.meanIntersectionOverUnion()

            da_acc_seg.update(da_acc, nb)
            da_IoU_seg.update(da_IoU, nb)
            da_mIoU_seg.update(da_mIoU, nb)

            ll_acc = ll_metric.lineAccuracy()  # 假设有这个方法
            ll_IoU = ll_metric.IntersectionOverUnion()
            ll_mIoU = ll_metric.meanIntersectionOverUnion()

            ll_acc_seg.update(ll_acc, nb)
            ll_IoU_seg.update(ll_IoU, nb)
            ll_mIoU_seg.update(ll_mIoU, nb)

            # 为下一个 batch 重置 metric 对象
            da_metric.reset()
            ll_metric.reset()
            ...
            if config.TEST.PLOTS:
                ...
                # -------------------- 请将以下新代码块完整粘贴到刚才删除的位置 --------------------
                if batch_i < 3:  # 只可视化前3个batch的结果，防止生成过多图片
                    for i in range(nb):  # nb 是当前batch的大小
                        img_test = cv2.imread(paths[i])
                        h0, w0 = img_test.shape[:2]  # 原始图像尺寸

                        # 获取当前样本的 padding 和 ratio
                        ratio = shapes[i][1][0]
                        pad = shapes[i][1][1]

                        # --- 可行驶区域 (DA) 可视化 ---
                        # 获取单个样本的预测和标签
                        da_predict_logits = da_seg_out[i]  # 形状 [C_da, H, W]
                        da_gt_single = target[1][i]  # 形状 [H, W]

                        # 1. 从 logits 中获取预测类别图
                        _, da_predict_map = torch.max(da_predict_logits, 0)  # 形状 [H, W]

                        # 2. 裁剪 padding
                        h, w = da_predict_map.shape  # letterbox 后的 H, W
                        pad_w, pad_h = int(pad[0]), int(pad[1])
                        da_predict_cropped = da_predict_map[pad_h:h - pad_h, pad_w:w - pad_w]
                        da_gt_cropped = da_gt_single[pad_h:h - pad_h, pad_w:w - pad_w]

                        # 3. 上采样（放大）到原始图像尺寸
                        #    使用 INTER_NEAREST 确保类别ID不被破坏
